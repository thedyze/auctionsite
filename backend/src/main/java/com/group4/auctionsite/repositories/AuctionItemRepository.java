...
public interface AuctionItemRepository extends JpaRepository<AuctionItem, Long> {
    ...
    @Query(value = "SELECT i.*,(SELECT MAX(bid) FROM bid WHERE item_id = i.id) AS highest_bid, (SELECT COUNT(item_id) FROM bid WHERE item_id = i.id ) AS number_of_bids FROM auction_item AS i, tag AS t, itemXtag AS x " +
            ...
            "AND highest_bid BETWEEN :priceFrom AND :priceTo " +
            "UNION " +
            "SELECT *, (SELECT MAX(bid) FROM bid WHERE item_id = i.id) AS highest_bid, (SELECT COUNT(item_id) FROM bid WHERE item_id = i.id ) AS number_of_bids FROM auction_item AS i " +
